...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "rCobros_FechasBancosCajas_CCSJ"
'-------------------------------------------------------------------------------------------'
Partial Class rCobros_FechasBancosCajas_CCSJ
	...
	Try
			...
			loConsulta.AppendLine("SELECT			(CASE WHEN Detalles_Cobros.Tip_Ope IN('Transferencia', 'Deposito') ")
			loConsulta.AppendLine("         		    THEN 'CUENTAS' ")
			loConsulta.AppendLine("         		    ELSE 'CAJAS' ")
			loConsulta.AppendLine("         		END)                                                        AS Grupo,	    ")
			loConsulta.AppendLine("         		(CASE WHEN Detalles_Cobros.Tip_Ope IN('Transferencia', 'Deposito') ")
			loConsulta.AppendLine("         		    THEN Detalles_Cobros.Cod_Cue ")
			loConsulta.AppendLine("         		    ELSE Detalles_Cobros.Cod_Caj ")
			loConsulta.AppendLine("         		END)                                                        AS Subgrupo,    ")
			loConsulta.AppendLine(" 				Cobros.Documento		                                    AS Documento,   ")
			loConsulta.AppendLine(" 				Cobros.Fec_Ini			                                    AS Fec_Ini,	    ")
			loConsulta.AppendLine(" 				Cobros.Cod_Cli			                                    AS Cod_Cli,     ")
			loConsulta.AppendLine(" 				CASE WHEN (Clientes.generico = 1 AND COALESCE(cuentas_cobrar.nom_cli, '')>'')")
			...
			loConsulta.AppendLine(" 				COALESCE(Banco_Detalle.Nom_Ban, Banco_Cuenta.nom_ban,'')    AS Nom_Ban,	    ")
			loConsulta.AppendLine(" 				COALESCE(Cuentas_Bancarias.Num_Cue, '')                     AS Numero_Cuenta,")
			...
			loConsulta.AppendLine("	    LEFT JOIN   Bancos Banco_Detalle ")
			loConsulta.AppendLine("	            ON  Banco_Detalle.Cod_Ban = Detalles_Cobros.Cod_Ban")
			loConsulta.AppendLine("	    LEFT JOIN   Bancos Banco_Cuenta ")
			loConsulta.AppendLine("	            ON  Banco_Cuenta.Cod_Ban = Cuentas_Bancarias.Cod_Ban")
            ...
            End if
            loConsulta.AppendLine("ORDER BY    CAST(Cobros.Fec_Ini AS DATE), ")
            loConsulta.AppendLine("            Grupo ASC, ")
            loConsulta.AppendLine("            Subgrupo ASC,")
            loConsulta.AppendLine("            Detalles_Cobros.Tip_Ope,")
            loConsulta.AppendLine("            Cobros.Fec_Ini, " & lcOrdenamiento)
            ...
            loObjetoReporte	=  cusAplicacion.goReportes.mCargarReporte("rCobros_FechasBancosCajas_CCSJ", laDatosReporte)
            
            Me.mTraducirReporte(loObjetoReporte)
            
			Me.mFormatearCamposReporte(loObjetoReporte)

            Me.crvrCobros_FechasBancosCajas_CCSJ.ReportSource =	 loObjetoReporte
...
' Fin del codigo                                                    			            '
'-------------------------------------------------------------------------------------------'
' RJG:  27/03/14: Programacion inicial, a partir de rCobros_Fechas_CCSJ.				    '
'-------------------------------------------------------------------------------------------'
