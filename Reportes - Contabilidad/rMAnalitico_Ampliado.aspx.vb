...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "rMAnalitico_Ampliado"
'-------------------------------------------------------------------------------------------'
Partial Class rMAnalitico_Ampliado
	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
		...
		'Establece las longitudes de todos los niveles
			...
			Else
				loComandoSeleccionar.AppendLine("		AND ISNULL(Renglones_Comprobantes.Cod_Cen, '')	BETWEEN " & lcCentroCostoDesde		& "	AND	" & lcCentroCostoHasta)
				loComandoSeleccionar.AppendLine("		AND ISNULL(Renglones_Comprobantes.Cod_Gas, '')	BETWEEN " & lcCuentaGastoDesde		& "	AND	" & lcCuentaGastoHasta)
				loComandoSeleccionar.AppendLine("		AND ISNULL(Renglones_Comprobantes.Cod_Aux, '')	BETWEEN " & lcAuxiliarDesde			& "	AND	" & lcAuxiliarHasta)
				loComandoSeleccionar.AppendLine("		AND ISNULL(Renglones_Comprobantes.Cod_Mon, '')	BETWEEN " & lcMonedaDesde			& "	AND	" & lcMonedaHasta)
			End If
		...
		'-------------------------------------------------------------------------------------------------------
			...
			loObjetoReporte = cusAplicacion.goReportes.mCargarReporte("rMAnalitico_Ampliado", laDatosReporte)

			Me.mTraducirReporte(loObjetoReporte)

			Me.mFormatearCamposReporte(loObjetoReporte)

			Me.crvrMAnalitico_Ampliado.ReportSource = loObjetoReporte
...
' Fin del codigo																			'
'-------------------------------------------------------------------------------------------'
' RJG: 09/11/11: Codigo inicial, a partir de Mayor Analítico.								'
'-------------------------------------------------------------------------------------------' 
' RJG: 11/11/11: Ajuste en el cálculo del acumulado.										'
'-------------------------------------------------------------------------------------------' 
