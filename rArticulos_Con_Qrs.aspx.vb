...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "rArticulos_Con_Qrs"
'-------------------------------------------------------------------------------------------'
Partial Class rArticulos_Con_Qrs
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loObjetoReporte = cusAplicacion.goReportes.mCargarReporte("rArticulos_Con_Qrs", laDatosReporte)

            Me.mTraducirReporte(loObjetoReporte)

            Me.mFormatearCamposReporte(loObjetoReporte)

            Me.crvrArticulos_Con_Qrs.ReportSource = loObjetoReporte
    ...
    Protected Function mObtenerImagenQR(ByVal lcCodigo As String) As Image


        Dim loEncoder As QRCodeEncoder = New QRCodeEncoder()


        Dim loImagen As Image
        Dim loBitmap As Bitmap

       
        loEncoder.QRCodeEncodeMode = QRCodeEncoder.ENCODE_MODE.BYTE
        loEncoder.QRCodeScale = 2
        loEncoder.QRCodeVersion = 5
        loEncoder.QRCodeErrorCorrect = ThoughtWorks.QRCode.Codec.QRCodeEncoder.ERROR_CORRECTION.M
        loImagen = loEncoder.Encode(lcCodigo)
        loBitmap = New Bitmap(loImagen)
    ...
    Protected Sub mCargarFoto(ByRef loTabla As DataTable)
        ...
        For j As Integer = 0 To (loTabla.Rows.Count - 1)
            ...
            Dim lcCodigo As String = loTabla.Rows(j).Item("Cod_Art").ToString()
            loImage = mObtenerImagenQR(lcCodigo)
