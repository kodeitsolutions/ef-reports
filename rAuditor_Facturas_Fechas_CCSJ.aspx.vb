...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "rAuditor_Facturas_Fechas_CCSJ"
'-------------------------------------------------------------------------------------------'
Partial Class rAuditor_Facturas_Fechas_CCSJ 
	...
	Try
			...
			loConsulta.AppendLine(" 				Facturas.Registro,")
			loConsulta.AppendLine(" 				Facturas.Usu_Cre,")
			loConsulta.AppendLine(" 				Facturas.Usu_Mod,")
			loConsulta.AppendLine(" 				Facturas.Equ_Cre,")
			loConsulta.AppendLine(" 				Facturas.Equ_Mod,")
			loConsulta.AppendLine(" 				Facturas.Reg_Mod,")
			loConsulta.AppendLine(" 				(CASE WHEN Facturas.Usu_Cre=''")
			loConsulta.AppendLine(" 				    THEN '[Sin usuario]'")
			loConsulta.AppendLine(" 				    ELSE RTRIM(Facturas.Usu_Cre)+' - ' ")
			loConsulta.AppendLine(" 				         + COALESCE(RTRIM(Creadores.Nom_Usu COLLATE SQL_Latin1_General_CP1_CI_AS), '[Desconocido]')")
			loConsulta.AppendLine(" 				    END) AS Creado_Por,")
			loConsulta.AppendLine(" 				(CASE WHEN Facturas.Usu_Mod=''")
			loConsulta.AppendLine(" 				    THEN '[Sin usuario]'")
			loConsulta.AppendLine(" 				    ELSE RTRIM(Facturas.Usu_Mod)+' - ' ")
			loConsulta.AppendLine(" 				         + COALESCE(RTRIM(Modificadores.Nom_Usu COLLATE SQL_Latin1_General_CP1_CI_AS), '[Desconocido]')")
			loConsulta.AppendLine(" 				    END) AS Modificado_Por")
			...
			loConsulta.AppendLine("    LEFT JOIN 	Factory_Global.dbo.Usuarios AS Creadores" )
			loConsulta.AppendLine("       ON 	    Creadores.Cod_Usu COLLATE DATABASE_DEFAULT = Facturas.Usu_Cre COLLATE DATABASE_DEFAULT " )
			loConsulta.AppendLine("    LEFT JOIN 	Factory_Global.dbo.Usuarios AS Modificadores" )
			loConsulta.AppendLine("       ON 	    Modificadores.Cod_Usu COLLATE DATABASE_DEFAULT = Facturas.Usu_Mod COLLATE DATABASE_DEFAULT " )
            ...
            loConsulta.AppendLine("             AND Facturas.Usu_Cre	BETWEEN " & lcParametro10Desde)
            loConsulta.AppendLine("    			    AND " & lcParametro10Hasta)
            loConsulta.AppendLine(" 			AND Facturas.Usu_Mod	BETWEEN" & lcParametro11Desde)
            ...
            loObjetoReporte	=  cusAplicacion.goReportes.mCargarReporte("rAuditor_Facturas_Fechas_CCSJ", laDatosReporte)
            
            Me.mTraducirReporte(loObjetoReporte)
            
			Me.mFormatearCamposReporte(loObjetoReporte)

            Me.crvrAuditor_Facturas_Fechas_CCSJ.ReportSource =	 loObjetoReporte	
...
' Fin del codigo																			'
'-------------------------------------------------------------------------------------------'
' RJG: 07/07/14: Codigo inicial, a partir de rFacturas_Fechas_CCSJ. Se agregaron campos de  '
'                auditorias básicas de las facturas.						                '
'-------------------------------------------------------------------------------------------'
