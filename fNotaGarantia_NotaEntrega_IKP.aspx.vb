...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "fNotaGarantia_NotaEntrega_IKP"
'-------------------------------------------------------------------------------------------'
Partial Class fNotaGarantia_NotaEntrega_IKP
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loConsulta.AppendLine("CREATE TABLE #tmpDocumento( Documento   CHAR(10) COLLATE DATABASE_DEFAULT, ")
            ...
            loConsulta.AppendLine("")
            loConsulta.AppendLine("")
            loConsulta.AppendLine("INSERT INTO #tmpDocumento(Documento, Fec_Ini, Cod_Cli, Nom_Cli, Rif, Dir_Fis, Telefonos,")
            ...
            loConsulta.AppendLine("SELECT  Entregas.Documento, ")
            loConsulta.AppendLine("        Entregas.Fec_Ini, ")
            loConsulta.AppendLine("        Entregas.cod_cli, ")
            loConsulta.AppendLine("        RTRIM(CASE WHEN Entregas.nom_cli<>'' THEN Entregas.nom_cli ELSE clientes.nom_cli END),")
            loConsulta.AppendLine("        RTRIM(CASE WHEN Entregas.rif<>'' THEN Entregas.rif ELSE clientes.rif END),")
            loConsulta.AppendLine("        RTRIM(CASE WHEN Entregas.Dir_Fis<>'' THEN Entregas.Dir_Fis ELSE clientes.Dir_Fis END),")
            loConsulta.AppendLine("        RTRIM(CASE WHEN Entregas.telefonos<>'' THEN Entregas.telefonos ELSE clientes.telefonos END),")
            ...
            loConsulta.AppendLine("        renglones_Entregas.notas,")
            ...
            loConsulta.AppendLine("        renglones_Entregas.por_imp1,")
            ...
            loConsulta.AppendLine("UPDATE #tmpDocumento")
            ...
            loConsulta.AppendLine("")
            loConsulta.AppendLine("SELECT      #tmpDocumento.*, COALESCE(seriales.serial, '') AS Serial ")
            loConsulta.AppendLine("FROM        #tmpDocumento")
            loConsulta.AppendLine("    LEFT JOIN seriales ")
            loConsulta.AppendLine("        ON  seriales.tip_sal = 'Entregas'")
            loConsulta.AppendLine("        AND seriales.doc_sal = #tmpDocumento.Documento")
            loConsulta.AppendLine("        AND seriales.ren_sal = #tmpDocumento.Renglon;")
            loConsulta.AppendLine("")
            loConsulta.AppendLine("DROP TABLE #tmpDocumento;")
            loConsulta.AppendLine("")
           ...
           'Me.mEscribirConsulta(loConsulta.ToString())
			...
			Dim llDocumentoImpreso As Boolean = Me.mGenerarDocumentoIPOSMXL(laDatosReporte.Tables(0))
...
''' <remarks></remarks>
	...
	'-------------------------------------------------------------------------------------------'	
        ...
        loNodo = loTextos.AppendChild(loSalidaXml.CreateElement("texto"))
        loNodo.SetAttribute("nombre", "texto" & loTextos.ChildNodes.Count.ToString("000"))
        loNodo.AppendChild(loSalidaXml.CreateCDataSection(Strings.Left("Origen: Nota de Entrega " & lcDocumento & "/Fecha: " & lcFecha & lcTamañoMAX, KN_COLUMNAS)))
	...
	'-------------------------------------------------------------------------------------------'	
        ...
        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "descargardocumentoXml", loScript.ToString(), True)
            ...
            Me.WbcAdministradorMensajeModal.mMostrarMensajeModal("Proceso Completado", _
                          "Se generó un archivo XML de Nota de Garantía (a partir de la nota de entrega seleccionada) para ser impreso con eFactory PLC.", _
                           vis3Controles.wbcAdministradorMensajeModal.enumTipoMensaje.KN_Informacion, _
                           "auto", _
                           "auto")
