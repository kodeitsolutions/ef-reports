...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "TRF_rBalance_General"
'-------------------------------------------------------------------------------------------'
Partial Class TRF_rBalance_General
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loComandoSeleccionar.AppendLine("			END")
            loComandoSeleccionar.AppendLine("		) AS Total")
            loComandoSeleccionar.AppendLine("INTO	#tmpPasivoCapital")
            loComandoSeleccionar.AppendLine("FROM	Cuentas_Contables ")
            loComandoSeleccionar.AppendLine("	LEFT OUTER JOIN Renglones_Comprobantes ON Cuentas_Contables.Cod_Cue = Renglones_Comprobantes.Cod_Cue ")
            loComandoSeleccionar.AppendLine("			AND (Renglones_Comprobantes.fec_ini <= @lcFechaHasta)")
            loComandoSeleccionar.AppendLine("WHERE	Cuentas_Contables.Movimiento=1")
            loComandoSeleccionar.AppendLine("	AND Cuentas_Contables.Categoria IN ('Pasivos', 'Capital')")
            If lnNivelMax = 5 Then
                ...
                loComandoSeleccionar.AppendLine("	(SELECT Total FROM #tmpPasivoCapital) AS Pasivo_Capital  ")
                loComandoSeleccionar.AppendLine("FROM #tmpBalance ")
                loComandoSeleccionar.AppendLine("ORDER BY Cod_Cue")
            Else
            ...
            loComandoSeleccionar.AppendLine("DROP TABLE #tmpPasivoCapital")
            ...
            loObjetoReporte = cusAplicacion.goReportes.mCargarReporte("TRF_rBalance_General", laDatosReporte)

            Me.mTraducirReporte(loObjetoReporte)

            Me.mFormatearCamposReporte(loObjetoReporte)

            Me.crvTRF_rBalance_General.ReportSource = loObjetoReporte
