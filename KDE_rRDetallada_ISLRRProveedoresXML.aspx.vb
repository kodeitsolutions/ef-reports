...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "KDE_rRDetallada_ISLRRProveedoresXML"
'-------------------------------------------------------------------------------------------'
Partial Class KDE_rRDetallada_ISLRRProveedoresXML
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loConsulta.AppendLine("         CASE WHEN Documentos.Fec_Ini < '20180820' AND Documentos.Fec_Ini > '20180731'")
            ...
            loConsulta.AppendLine("         CASE WHEN Documentos.Fec_Ini < '20180820' AND Documentos.Fec_Ini > '20180731'")
            loConsulta.AppendLine("              THEN (Retenciones_Documentos.Mon_Ret/100000)")
            loConsulta.AppendLine("              ELSE Retenciones_Documentos.Mon_Ret")
            loConsulta.AppendLine("         END                                     AS Monto_Retenido,")
            ...
            loConsulta.AppendLine("         CASE WHEN Ordenes_Pagos.Fec_Ini < '20180820' AND Ordenes_Pagos.Fec_Ini > '20180731'")
            loConsulta.AppendLine("              THEN (Renglones_OPagos.Mon_Net/100000)")
            loConsulta.AppendLine("              ELSE Renglones_OPagos.Mon_Net")
            loConsulta.AppendLine("         END                                         AS Base_Retencion,")
            loConsulta.AppendLine("         COALESCE((SELECT CASE WHEN RTRIM(R_OP.Comentario) <> ''")
            loConsulta.AppendLine("                               THEN CAST(R_OP.Comentario AS DECIMAL (28,2))")
            loConsulta.AppendLine("                               ELSE @lnCero  END")
            ...
            'loConsulta.AppendLine("         COALESCE((SELECT R_OP.Mon_Net")
            'loConsulta.AppendLine("                   FROM Renglones_OPagos AS R_OP")
            'loConsulta.AppendLine("                   WHERE R_OP.Cod_Con = 'P0007'")
            'loConsulta.AppendLine("                     AND R_OP.Documento = Renglones_OPagos.Documento")
            'loConsulta.AppendLine("         ),@lnCero)                                 AS Monto_Retenido,")
            loConsulta.AppendLine("       COALESCE((SELECT CASE WHEN Ordenes_Pagos.Fec_Ini < '20180820' AND Ordenes_Pagos.Fec_Ini > '20180731'")
            loConsulta.AppendLine("                             THEN (R_OP.Mon_Net/100000)")
            loConsulta.AppendLine("                             ELSE R_OP.Mon_Net END")
            loConsulta.AppendLine("                 FROM Renglones_OPagos AS R_OP")
            loConsulta.AppendLine("                 WHERE R_OP.Cod_Con = 'P0007'")
            loConsulta.AppendLine("                   AND R_OP.Documento = Renglones_OPagos.Documento")
            loConsulta.AppendLine("               ),@lnCero)                            AS Monto_Retenido,")
            ...
            loConsulta.AppendLine("WHERE Renglones_OPagos.Cod_Con = 'E0102'")
            loConsulta.AppendLine("     AND Ordenes_Pagos.Status <> 'Anulado'")
            loConsulta.AppendLine("     AND Ordenes_Pagos.Fec_Ini BETWEEN @ldFecha_Desde AND @ldFecha_Hasta")
            loConsulta.AppendLine("")
            'loConsulta.AppendLine("UNION ALL		")
            'loConsulta.AppendLine("")
            'loConsulta.AppendLine("SELECT	CASE WHEN DAY(Cuentas_Pagar.Fec_Ini) <= 15")
            'loConsulta.AppendLine("              THEN CONCAT('15',REPLACE(RIGHT(CONVERT(VARCHAR(10),Cuentas_Pagar.Fec_Ini, 103), 7),'/',''))")
            'loConsulta.AppendLine("              ELSE CONCAT(CAST(DAY(DATEADD(MS,- 3,DATEADD(MM,0,DATEADD(M,DATEDIFF(MM,0,Cuentas_Pagar.Fec_Ini)+1,0)))) AS VARCHAR(2)),REPLACE(RIGHT(CONVERT(VARCHAR(10),Cuentas_Pagar.Fec_Ini, 103), 7),'/',''))")
            'loConsulta.AppendLine("         END										AS Factura_Origen,")
            'loConsulta.AppendLine("     	CASE WHEN DAY(Cuentas_Pagar.Fec_Ini) <= 15")
            'loConsulta.AppendLine("              THEN CONCAT('15',REPLACE(RIGHT(CONVERT(VARCHAR(10),Cuentas_Pagar.Fec_Ini, 103), 7),'/',''))")
            'loConsulta.AppendLine("              ELSE CONCAT(CAST(DAY(DATEADD(MS,- 3,DATEADD(MM,0,DATEADD(M,DATEDIFF(MM,0,Cuentas_Pagar.Fec_Ini)+1,0)))) AS VARCHAR(2)),REPLACE(RIGHT(CONVERT(VARCHAR(10),Cuentas_Pagar.Fec_Ini, 103), 7),'/',''))")
            'loConsulta.AppendLine("         END										AS Control_Origen,")
            'loConsulta.AppendLine("			Cuentas_Pagar.Fec_Ini					AS Fecha_Retencion,")
            'loConsulta.AppendLine("			Campos_Extras.Doble1					AS Base_Retencion,")
            'loConsulta.AppendLine("			Campos_Extras.Doble2					AS Porcentaje_Retenido,")
            'loConsulta.AppendLine("			'1'										AS Codigo_Concepto,")
            'loConsulta.AppendLine("			Cuentas_Pagar.Mon_Net					AS Monto_Retenido,")
            'loConsulta.AppendLine("			Proveedores.Rif							AS Rif")
            'loConsulta.AppendLine("FROM	Cuentas_Pagar")
            'loConsulta.AppendLine("     JOIN Campos_Extras ON Cuentas_Pagar.Documento = Campos_Extras.Cod_Reg")
            'loConsulta.AppendLine("			AND Origen = 'Cuentas_Pagar' AND Clase = 'ISLR'")
            'loConsulta.AppendLine("		JOIN Proveedores ON Proveedores.Cod_Pro = Cuentas_Pagar.Cod_Pro")
            'loConsulta.AppendLine("WHERE Cuentas_Pagar.Cod_Tip = 'ISLR'")
            'loConsulta.AppendLine("     AND	Cuentas_Pagar.Status <> 'Anulado'")
            'loConsulta.AppendLine("     AND Cuentas_Pagar.Fec_Ini BETWEEN @ldFecha_Desde AND @ldFecha_Hasta")
            ...
            'loConsulta.AppendLine("			),(SELECT SUM(Renglones_Recibos.Mon_Net")
            'loConsulta.AppendLine("				FROM Renglones_Recibos")
            'loConsulta.AppendLine("				WHERE Documento = Recibos.Documento")
            'loConsulta.AppendLine("					AND Tipo = 'Asignacion'),@lnCero)						AS Base_Retencion,")
            loConsulta.AppendLine("			COALESCE((SELECT CASE WHEN Recibos.Fec_Ini < '20180820' AND Recibos.Fec_Ini > '20180731'")
            ...
            loConsulta.AppendLine("				      WHERE Documento = Recibos.Documento")
            loConsulta.AppendLine("					    AND Cod_Con IN ('R005', 'R405')")
            loConsulta.AppendLine("			),(SELECT SUM(CASE WHEN Renglones_Recibos.Fec_Ini < '20180820' AND Renglones_Recibos.Fec_Ini > '20180731'")
            ...
            loConsulta.AppendLine("				WHERE Documento = Recibos.Documento")
            loConsulta.AppendLine("					AND Tipo = 'Asignacion'),@lnCero)						AS Base_Retencion,")
            loConsulta.AppendLine("			COALESCE((SELECT CAST (SUBSTRING(Renglones_Recibos.val_car,0, LEN(Renglones_Recibos.val_car)) AS DECIMAL (28,2))")
            loConsulta.AppendLine("					  FROM Renglones_Recibos")
            loConsulta.AppendLine("					  WHERE Documento = Recibos.Documento")
            loConsulta.AppendLine("							AND Cod_Con IN ('R005', 'R405')")
            loConsulta.AppendLine("			),@lnCero)														AS Porcentaje_Retenido,")
            loConsulta.AppendLine("			'1'																AS Codigo_Concepto,")
            'loConsulta.AppendLine("			COALESCE((SELECT Renglones_Recibos.Mon_Net")
            'loConsulta.AppendLine("					  FROM Renglones_Recibos")
            'loConsulta.AppendLine("					  WHERE Documento = Recibos.Documento")
            'loConsulta.AppendLine("						    AND Cod_Con IN ('R005', 'R405')")
            'loConsulta.AppendLine("			),@lnCero)														AS Monto_Retenido,")
            loConsulta.AppendLine("			COALESCE((SELECT CASE WHEN Renglones_Recibos.Fec_Ini < '20180820' AND Renglones_Recibos.Fec_Ini > '20180731'")
            loConsulta.AppendLine("                               THEN (Renglones_Recibos.Mon_Net/100000)")
            loConsulta.AppendLine("                               ELSE Renglones_Recibos.Mon_Net")
            loConsulta.AppendLine("                         END")
            loConsulta.AppendLine("					  FROM Renglones_Recibos")
            ...
            loConsulta.AppendLine(" AND Recibos.Cod_Con IN ('01','92')")
            ...
            Me.Response.AppendHeader("content-disposition", "attachment; filename=RelacionRetencionesISLR_KODE_" & lcPeriodo & ".xml")
