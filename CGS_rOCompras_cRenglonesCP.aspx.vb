...
Partial Class CGS_rOCompras_cRenglonesCP
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            lcComandoSeleccionar.AppendLine("		COALESCE(Renglones_Recepciones.Can_Art1,0)					            AS CanArt_R,")
            lcComandoSeleccionar.AppendLine("		COALESCE((CASE WHEN RTRIM(Renglones_Recepciones.Comentario) <> ''")
            ...
            lcComandoSeleccionar.AppendLine("                  END) ,0)                                                     AS Cantidad_R,")
            lcComandoSeleccionar.AppendLine("       ")
            lcComandoSeleccionar.AppendLine("       ")
            lcComandoSeleccionar.AppendLine("       ")
            lcComandoSeleccionar.AppendLine("       ")
            'lcComandoSeleccionar.AppendLine("		COALESCE(Operaciones_Lotes.Cod_Lot,'')									AS Lote,")
            lcComandoSeleccionar.AppendLine("       CASE WHEN Recibido.Usa_Lot = 1")
            lcComandoSeleccionar.AppendLine("            THEN COALESCE(Operaciones_Lotes.Cod_Lot,'')")
            lcComandoSeleccionar.AppendLine("            ELSE '' END                                                        AS Lote,")
            'lcComandoSeleccionar.AppendLine("		COALESCE(Operaciones_Lotes.Cantidad,0)						            AS Cantidad_Lote,")
            lcComandoSeleccionar.AppendLine("       CASE WHEN Recibido.Usa_Lot = 1")
            lcComandoSeleccionar.AppendLine("            THEN COALESCE(Operaciones_Lotes.Cantidad,0)")
            lcComandoSeleccionar.AppendLine("            ELSE 0 END                                                         AS Cantidad_Lote,")
            ...
            lcComandoSeleccionar.AppendLine("            OR Renglones_Recepciones.Cod_Art = Renglones_OCompras.Caracter1)")
            lcComandoSeleccionar.AppendLine("   LEFT JOIN Articulos AS Recibido ON Renglones_Recepciones.Cod_Art = Recibido.Cod_Art")
