Imports System.Data
Partial Class rCotizaciones_HPrecios
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            'loComandoSeleccionar.AppendLine("			CONVERT(NCHAR(10), Cotizaciones.Fec_Ini, 103)		AS	Fec_Ini, ")
            'loComandoSeleccionar.AppendLine("			CONVERT(NCHAR(10), Cotizaciones.Fec_Ini, 103)		AS	Fec_Ini1, ")
            'loComandoSeleccionar.AppendLine("			CONVERT(NCHAR(10), Cotizaciones.Fec_Ini, 112)     AS	Fec_Ini2, ")
            ...
            loComandoSeleccionar.AppendLine("           And Articulos.Cod_Art           =   Renglones_Cotizaciones.Cod_Art ")
            loComandoSeleccionar.AppendLine("           And Almacenes.Cod_Alm           =   Renglones_Cotizaciones.Cod_Alm ")
            ...
            loComandoSeleccionar.AppendLine(" ORDER BY  Cotizaciones.Cod_Cli, Renglones_Cotizaciones.Cod_Art, Cotizaciones.Fec_Ini ")
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''            
            ...
            loComandoSeleccionar.AppendLine(" DECLARE @Fec_Ini2	INTEGER ")
            loComandoSeleccionar.AppendLine(" DECLARE @Precio1	FLOAT ")
            loComandoSeleccionar.AppendLine(" DECLARE @Cod_Cli_Ant	CHAR(30) ")
            loComandoSeleccionar.AppendLine(" DECLARE @Aux	INTEGER  ")
            loComandoSeleccionar.AppendLine(" SET @Aux = 1 ")
            
            loComandoSeleccionar.AppendLine(" FETCH NEXT FROM curPrueba INTO @Cod_Cli, @Cod_Art, @Fec_Ini1, @Fec_Ini2, @Precio1 ")
            loComandoSeleccionar.AppendLine(" WHILE @@FETCH_STATUS = 0 ")
            loComandoSeleccionar.AppendLine(" BEGIN ")
            
            loComandoSeleccionar.AppendLine(" IF @Aux = 1 ")
            loComandoSeleccionar.AppendLine(" BEGIN ")
            loComandoSeleccionar.AppendLine(" 	SET @Cod_Cli_Ant = @Cod_Cli ")
            loComandoSeleccionar.AppendLine(" 	SET @Aux = 2 ")
            loComandoSeleccionar.AppendLine(" END  ")
            
            loComandoSeleccionar.AppendLine("	IF @Cod_Cli_Ant = @Cod_Cli ")
            loComandoSeleccionar.AppendLine("		BEGIN ")
            loComandoSeleccionar.AppendLine(" 			IF @Precio1 <> @Pre_Ant ")
            loComandoSeleccionar.AppendLine("				BEGIN ")
            loComandoSeleccionar.AppendLine(" 				      INSERT INTO #curTemporal3	(Cod_Cli, Cod_Art, Fec_Ini1, Fec_Ini2, Precio1, Cam_Eli) ")
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''            
            ...
            loObjetoReporte = cusAplicacion.goReportes.mCargarReporte("rCotizaciones_HPrecios", laDatosReporte)

            Me.mTraducirReporte(loObjetoReporte)

            Me.mFormatearCamposReporte(loObjetoReporte)

            Me.crvrCotizaciones_HPrecios.ReportSource = loObjetoReporte
...
' Fin del codigo
'-------------------------------------------------------------------------------------------'
' JJD: 28/02/09: Codigo inicial.
'-------------------------------------------------------------------------------------------'
' CMS:  18/08/09: Metodo de ordenamiento, verificacionde registros
'-------------------------------------------------------------------------------------------'
' CMS:  10/09/09: Se agrego el filtro estatus de cliente
'-------------------------------------------------------------------------------------------'
' CMS:  10/09/09: En el procesamiento para determinar el ultimo precio se inicializo el 
'					cliente cada vez que cambie.
'-------------------------------------------------------------------------------------------'