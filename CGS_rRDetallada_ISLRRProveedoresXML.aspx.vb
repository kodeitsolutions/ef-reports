...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "CGS_rRDetallada_ISLRRProveedoresXML"
'-------------------------------------------------------------------------------------------'
Partial Class CGS_rRDetallada_ISLRRProveedoresXML
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loConsulta.AppendLine("SELECT      (CASE WHEN DAY(Renglones_Recibos.Fec_Ini) <= 15")
            loConsulta.AppendLine("                    THEN (CASE WHEN MONTH(Renglones_Recibos.Fec_Ini) < 10")
            loConsulta.AppendLine("                            THEN CONCAT('15','0',MONTH(Renglones_Recibos.Fec_Ini),YEAR(Renglones_Recibos.Fec_Ini))")
            loConsulta.AppendLine("                            ELSE CONCAT('15',MONTH(Renglones_Recibos.Fec_Ini),YEAR(Renglones_Recibos.Fec_Ini))")
            loConsulta.AppendLine("                        END)")
            loConsulta.AppendLine("                    ELSE (CASE WHEN MONTH(Renglones_Recibos.Fec_Ini) < 10")
            loConsulta.AppendLine("                            THEN CONCAT('30','0',MONTH(Renglones_Recibos.Fec_Ini),YEAR(Renglones_Recibos.Fec_Ini))")
            loConsulta.AppendLine("                            ELSE CONCAT('30',MONTH(Renglones_Recibos.Fec_Ini),YEAR(Renglones_Recibos.Fec_Ini))")
            loConsulta.AppendLine("                            END)")
            loConsulta.AppendLine("                END)                                    AS Factura_Origen,")
            loConsulta.AppendLine("                (CASE WHEN DAY(Renglones_Recibos.Fec_Ini) <= 15")
            loConsulta.AppendLine("                    THEN (CASE WHEN MONTH(Renglones_Recibos.Fec_Ini) < 10")
            loConsulta.AppendLine("                            THEN CONCAT('15','0',MONTH(Renglones_Recibos.Fec_Ini),YEAR(Renglones_Recibos.Fec_Ini))")
            loConsulta.AppendLine("                            ELSE CONCAT('15',MONTH(Renglones_Recibos.Fec_Ini),YEAR(Renglones_Recibos.Fec_Ini))")
            loConsulta.AppendLine("                        END)")
            ...
            loConsulta.AppendLine("                Renglones_Recibos.Fec_Ini               AS Fecha_Retencion,")
            loConsulta.AppendLine("                '-'                                     AS Tipo_Pago,")
            loConsulta.AppendLine("                '-'                                     AS Numero_Pago,")
            loConsulta.AppendLine("                ROUND(Renglones_Recibos.Mon_Net * 100 / CAST (SUBSTRING(Renglones_Recibos.val_car,0, LEN(Renglones_Recibos.val_car)) AS DECIMAL (28,2)),2)")
            loConsulta.AppendLine("                 + (CASE WHEN NOT EXISTS ( SELECT       ROUND(Renglones_Recibos.Mon_Net * 100 / CAST (SUBSTRING(Renglones_Recibos.val_car,0, LEN(Renglones_Recibos.val_car)) AS DECIMAL (28,2)),2) ")
            loConsulta.AppendLine("                                        FROM         Renglones_Recibos")
            loConsulta.AppendLine("                                        JOIN    Recibos ON Renglones_Recibos.documento = Recibos.documento")
            loConsulta.AppendLine("                                            AND Renglones_Recibos.cod_con = 'R405'")
            loConsulta.AppendLine("                                        JOIN    Trabajadores ON Recibos.Cod_Tra = Trabajadores.Cod_Tra")
            loConsulta.AppendLine("                                            AND Trabajadores.Cod_Tra = TraOut.Cod_Tra")
            loConsulta.AppendLine("                                         WHERE Renglones_Recibos.Fec_Ini BETWEEN " & lcParametro0Desde)
            loConsulta.AppendLine("       		                            AND " & lcParametro0Hasta & ")")
            loConsulta.AppendLine("                THEN 0 ELSE ( SELECT       ROUND(Renglones_Recibos.Mon_Net * 100 / CAST (SUBSTRING(Renglones_Recibos.val_car,0, LEN(Renglones_Recibos.val_car)) AS DECIMAL (28,2)),2) ")
            loConsulta.AppendLine("                                        FROM         Renglones_Recibos")
            loConsulta.AppendLine("                                        JOIN    Recibos ON Renglones_Recibos.documento = Recibos.documento")
            loConsulta.AppendLine("                                            AND Renglones_Recibos.cod_con = 'R405'")
            loConsulta.AppendLine("                                        JOIN    Trabajadores ON Recibos.Cod_Tra = Trabajadores.Cod_Tra")
            ...
            loConsulta.AppendLine("                CAST (SUBSTRING(Renglones_Recibos.val_car,0, LEN(Renglones_Recibos.val_car)) AS DECIMAL (28,2)) AS Porcentaje_Retenido,")
            loConsulta.AppendLine("                '1'                                     AS Codigo_Concepto,")
            loConsulta.AppendLine("                Renglones_Recibos.Mon_Net               ")
            loConsulta.AppendLine("                + (CASE WHEN NOT EXISTS ( SELECT       Renglones_Recibos.Mon_Net")
            loConsulta.AppendLine("                                        FROM         Renglones_Recibos")
            loConsulta.AppendLine("                                        JOIN    Recibos ON Renglones_Recibos.documento = Recibos.documento")
            loConsulta.AppendLine("                                            AND Renglones_Recibos.cod_con = 'R405'")
            loConsulta.AppendLine("                                        JOIN    Trabajadores ON Recibos.Cod_Tra = Trabajadores.Cod_Tra")
            ...
            loConsulta.AppendLine("                                         WHERE Renglones_Recibos.Fec_Ini BETWEEN " & lcParametro0Desde)
            loConsulta.AppendLine("       		                            AND " & lcParametro0Hasta & ")")
            loConsulta.AppendLine("                THEN 0 ELSE ( SELECT       Renglones_Recibos.Mon_Net")
            loConsulta.AppendLine("                                        FROM         Renglones_Recibos")
            loConsulta.AppendLine("                                        JOIN    Recibos ON Renglones_Recibos.documento = Recibos.documento")
            loConsulta.AppendLine("                                            AND Renglones_Recibos.cod_con = 'R405'")
            loConsulta.AppendLine("                                        JOIN    Trabajadores ON Recibos.Cod_Tra = Trabajadores.Cod_Tra")
            ...
            loConsulta.AppendLine("                TraOut.Rif                      AS Rif")
            loConsulta.AppendLine("FROM            Renglones_Recibos")
            loConsulta.AppendLine("        JOIN    Recibos ON Renglones_Recibos.documento = Recibos.documento")
            loConsulta.AppendLine("            AND Renglones_Recibos.cod_con = 'R005'")
            loConsulta.AppendLine("        JOIN    Trabajadores AS TraOut ON Recibos.Cod_Tra = TraOut.Cod_Tra")
