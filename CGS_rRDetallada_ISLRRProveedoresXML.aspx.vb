...
Partial Class CGS_rRDetallada_ISLRRProveedoresXML
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loConsulta.AppendLine("                    THEN (CASE WHEN MONTH(Recibos.Fecha) < 10")
            loConsulta.AppendLine("                            THEN CONCAT('15','0',MONTH(Recibos.Fecha),YEAR(Recibos.Fecha))")
            loConsulta.AppendLine("                            ELSE CONCAT('15',MONTH(Recibos.Fecha),YEAR(Recibos.Fecha))")
            loConsulta.AppendLine("                        END)")
            loConsulta.AppendLine("                    ELSE (CASE WHEN MONTH(Recibos.Fecha) < 10")
            loConsulta.AppendLine("                            THEN CONCAT('30','0',MONTH(Recibos.Fecha),YEAR(Recibos.Fecha))")
            loConsulta.AppendLine("                            ELSE CONCAT('30',MONTH(Recibos.Fecha),YEAR(Recibos.Fecha))")
            loConsulta.AppendLine("                            END)")
