...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "fFacturas_Clinicas_MediaPagina_CCSJ"
'-------------------------------------------------------------------------------------------'
Partial Class fFacturas_Clinicas_MediaPagina_CCSJ
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loConsulta.AppendLine("-- ******************************************************************************************")
            loConsulta.AppendLine("-- Encabezado de la Factura.                                                                *")
            loConsulta.AppendLine("-- ******************************************************************************************")
            ...
            loConsulta.AppendLine("                            Rif_Cliente             VARCHAR(MAX) COLLATE DATABASE_DEFAULT,")
            ...
            loConsulta.AppendLine("                            Generico_Nombre         CHAR(100) COLLATE DATABASE_DEFAULT DEFAULT(''),")
            loConsulta.AppendLine("                            Generico_Rif            CHAR(20) COLLATE DATABASE_DEFAULT DEFAULT(''),")
            loConsulta.AppendLine("                            Generico_Direccion      VARCHAR(MAX) COLLATE DATABASE_DEFAULT DEFAULT(''),")
            loConsulta.AppendLine("                            Generico_Telefono       VARCHAR(100) COLLATE DATABASE_DEFAULT DEFAULT('') ")
            loConsulta.AppendLine("                            );")
            loConsulta.AppendLine("")
            loConsulta.AppendLine("")
            ...
            loConsulta.AppendLine("                            Generico_Nombre, Generico_Rif, Generico_Direccion, Generico_Telefono)")
            ...
            loConsulta.AppendLine("        nom_cli, rif, dir_fis, telefonos")
            ...
            loConsulta.AppendLine("")
            loConsulta.AppendLine("-- ******************************************************************************************")
            loConsulta.AppendLine("-- CASO ESPECIAL: EL CLIENTE DE LA FACTURA ES GENÉRICO.                                     *")
            loConsulta.AppendLine("-- EN ESTE CASO NO HAY SEGURO NI REPRESENTANTE: ES UN CLIENTE GENÉRICO COMÚN.               *")
            loConsulta.AppendLine("-- ******************************************************************************************")
            loConsulta.AppendLine("IF( (SELECT Generico FROM clientes WHERE cod_cli = @lcCliente) = 1)")
            loConsulta.AppendLine("BEGIN ")
            loConsulta.AppendLine("    ")
            loConsulta.AppendLine("    UPDATE  #tmpFactura")
            loConsulta.AppendLine("    SET     Paciente_Codigo = Clientes.Cod_Cli,")
            loConsulta.AppendLine("            Paciente_Nombre = Generico_Nombre,")
            loConsulta.AppendLine("            Paciente_Rif = Generico_Rif,")
            loConsulta.AppendLine("            Direccion = Generico_Direccion,")
            ...
            loConsulta.AppendLine("-- ******************************************************************************************")
            loConsulta.AppendLine("-- CASO CLINICO NORMAL: EL CLIENTE DE LA FACTURA NO ES GENÉRICO.                            * ")
            loConsulta.AppendLine("-- EN ESTE CASO HAY QUE BUSCAR LOS DATOS DEL SEGURO Y REPRESENTANTE, SI LOS HAY.            *")
            loConsulta.AppendLine("-- ******************************************************************************************")
            ...
            loConsulta.AppendLine("")
            loConsulta.AppendLine("-- ******************************************************************************************")
            loConsulta.AppendLine("-- OBTIENE EL NOMBRE DEL CLIENTE DE LA FACTURA.                                             *")
            loConsulta.AppendLine("-- ******************************************************************************************")
            loConsulta.AppendLine("UPDATE  #tmpFactura")
            ...
            loConsulta.AppendLine("        END ),")
            loConsulta.AppendLine("        Rif_Cliente = ")
            loConsulta.AppendLine("        (CASE WHEN RTRIM(Seguro_Nombre) > ''")
            loConsulta.AppendLine("             THEN Seguro_Rif ")
            loConsulta.AppendLine("             ELSE Paciente_Rif ")
            loConsulta.AppendLine("        END )")
            loConsulta.AppendLine("")
            loConsulta.AppendLine("")
            loConsulta.AppendLine("-- ******************************************************************************************")
            ...
            loConsulta.AppendLine("")
            loConsulta.AppendLine("-- ******************************************************************************************")
            loConsulta.AppendLine("-- Renglones de la factura.                                                                 *")
            loConsulta.AppendLine("-- ******************************************************************************************")
            ...
            loConsulta.AppendLine("            (CASE WHEN A.Generico = 1  ")
            loConsulta.AppendLine("                THEN R.Notas  ")
            loConsulta.AppendLine("                ELSE A.Nom_Art ")
            ...
            loConsulta.AppendLine("-- ******************************************************************************************")
            loConsulta.AppendLine("SELECT  #tmpFactura.Documento                           AS Documento, ")
            loConsulta.AppendLine("        #tmpFactura.Control                             AS Control, ")
            loConsulta.AppendLine("        #tmpFactura.Fec_Ini                             AS Fec_Ini, ")
            loConsulta.AppendLine("        #tmpFactura.Fec_Fin                             AS Fec_Fin, ")
            loConsulta.AppendLine("        #tmpFactura.Nombre_Cliente                      AS Nombre_Cliente, ")
            loConsulta.AppendLine("        #tmpFactura.Rif_Cliente                         AS Rif_Cliente, ")
            loConsulta.AppendLine("        #tmpFactura.Dias_Hospitalizacion                AS Dias_Hospitalizacion, ")
            loConsulta.AppendLine("        #tmpFactura.Horas_Aproximadas                   AS Horas_Aproximadas, ")
            loConsulta.AppendLine("        #tmpFactura.Comentario_Documento                AS Comentario_Documento, ")
            loConsulta.AppendLine("        #tmpFactura.Diagnostico                         AS Diagnostico, ")
            loConsulta.AppendLine("        #tmpFactura.Intervencion                        AS Intervencion, ")
            loConsulta.AppendLine("        #tmpFactura.Paciente_Codigo                     AS Paciente_Codigo, ")
            loConsulta.AppendLine("        (RTRIM(#tmpFactura.Paciente_Codigo) + ' - ' ")
            loConsulta.AppendLine("            + #tmpFactura.Paciente_Nombre)              AS Paciente_Nombre,")
            loConsulta.AppendLine("        #tmpFactura.Paciente_Rif                         AS Paciente_Rif, ")
            loConsulta.AppendLine("        #tmpFactura.Representante_Codigo, ")
            loConsulta.AppendLine("        (RTRIM(#tmpFactura.Representante_Codigo) + ' - '")
            loConsulta.AppendLine("            + #tmpFactura.Representante_Nombre)         AS Representante_Nombre,")
            loConsulta.AppendLine("        #tmpFactura.Representante_Rif                   AS Representante_Rif, ")
            loConsulta.AppendLine("        #tmpFactura.Seguro_Codigo,")
            loConsulta.AppendLine("        #tmpFactura.Seguro_Rif                          AS Seguro_Rif, ")
            loConsulta.AppendLine("        (RTRIM(#tmpFactura.Seguro_Codigo) + ' - ' ")
            loConsulta.AppendLine("            + #tmpFactura.Seguro_Nombre)                AS Seguro_Nombre,")
            loConsulta.AppendLine("        #tmpFactura.Medico_Codigo,")
            loConsulta.AppendLine("        (RTRIM(#tmpFactura.Medico_Codigo) + ' - ' ")
            loConsulta.AppendLine("            + #tmpFactura.Medico_Nombre)                AS Medico_Nombre,")
            loConsulta.AppendLine("        #tmpFactura.Direccion                           AS Direccion,  ")
            loConsulta.AppendLine("        #tmpFactura.Total_Bruto                         AS Total_Bruto, ")
            loConsulta.AppendLine("        #tmpFactura.Total_Impuesto                      AS Total_Impuesto,")
            loConsulta.AppendLine("        #tmpFactura.Total_Neto                          AS Total_Neto,")
            loConsulta.AppendLine("        #tmpRenglones.Renglon                           AS Renglon, ")
            loConsulta.AppendLine("        #tmpRenglones.Cod_Art                           AS Cod_Art, ")
            loConsulta.AppendLine("        #tmpRenglones.Nom_Art                           AS Nom_Art, ")
            loConsulta.AppendLine("        #tmpRenglones.Cod_Hon                           AS Cod_Hon, ")
            loConsulta.AppendLine("        #tmpRenglones.Nom_Hon                           AS Nom_Hon, ")
            loConsulta.AppendLine("        #tmpRenglones.Cantidad                          AS Cantidad, ")
            loConsulta.AppendLine("        #tmpRenglones.Bruto                             AS Bruto, ")
            loConsulta.AppendLine("        #tmpRenglones.Por_Imp                           AS Por_Imp, ")
            loConsulta.AppendLine("        #tmpRenglones.Impuesto                          AS Impuesto, ")
            loConsulta.AppendLine("        #tmpRenglones.Neto                              AS Neto, ")
            loConsulta.AppendLine("        #tmpRenglones.Cod_Dep                           AS Cod_Dep, ")
            loConsulta.AppendLine("        #tmpRenglones.Nom_Dep                           AS Nom_Dep, ")
            loConsulta.AppendLine("        #tmpFactura.Paciente_Rif                        AS Paciente_Rif,")
            loConsulta.AppendLine("        #tmpFactura.Seguro_Rif                          AS Seguro_Rif,")
            loConsulta.AppendLine("        (CASE WHEN Seguro_Rif>'' ")
            loConsulta.AppendLine("            THEN Seguro_Rif ")
            loConsulta.AppendLine("            ELSE Paciente_Rif ")
            loConsulta.AppendLine("        END)                                            AS Rif_Cliente")
            loConsulta.AppendLine("FROM    #tmpFactura")
            loConsulta.AppendLine("        CROSS JOIN #tmpRenglones")
            loConsulta.AppendLine("ORDER BY #tmpRenglones.Renglon")
            loConsulta.AppendLine("")
            ...
            loObjetoReporte = cusAplicacion.goFormatos.mCargarInforme("fFacturas_Clinicas_MediaPagina_CCSJ", laDatosReporte)

            Me.mTraducirReporte(loObjetoReporte)
            Me.mFormatearCamposReporte(loObjetoReporte)
            Me.crvfFacturas_Clinicas_MediaPagina_CCSJ.ReportSource = loObjetoReporte
...
'-------------------------------------------------------------------------------------------'
' JFP: 10/12/13: Adecuacion inicial para el tamaño de media pagina para emergencias         '
'-------------------------------------------------------------------------------------------'
' JJD: 28/01/14: Ajustes varios.                                                            '
'-------------------------------------------------------------------------------------------'
' RJG: 14/02/14: Se ajustaron los datos del representante y del Seguro.                     '
'-------------------------------------------------------------------------------------------'
' RJG: 15/02/14: Se agregaron los datos del cliente genérico y del artículo genérico.       '
'-------------------------------------------------------------------------------------------'
' RJG: 03/04/14: Ajuste en la dirección: ahora toma la dirección del seguro, sino la del    '
'                representante, y si no hay la del paciente.                                '
'-------------------------------------------------------------------------------------------'
