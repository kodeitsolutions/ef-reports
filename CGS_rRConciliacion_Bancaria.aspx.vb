...
Partial Class CGS_rRConciliacion_Bancaria
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loComandoSeleccionar.AppendLine("            SUM(Movimientos_Cuentas.Mon_Deb) - SUM(Movimientos_Cuentas.Mon_Hab)  AS Mon_Sal_Ini")
            ...
            loComandoSeleccionar.AppendLine("	   OR (Movimientos_Cuentas.Fec_Ini < @ldFechaInicio ")
            ...
            loComandoSeleccionar.AppendLine("")
            'MOVIMIENTOS QUE VIENEN DE PAGOS
            loComandoSeleccionar.AppendLine(" SELECT    Movimientos_Cuentas.Documento, ")
            loComandoSeleccionar.AppendLine("           Movimientos_Cuentas.Cod_Cue, ")
            loComandoSeleccionar.AppendLine("           Movimientos_Cuentas.Fec_Ini, ")
            loComandoSeleccionar.AppendLine("           Movimientos_Cuentas.Referencia, ")
            ...
            loComandoSeleccionar.AppendLine("           Movimientos_Cuentas.Mon_Deb, ")
            loComandoSeleccionar.AppendLine("           Movimientos_Cuentas.Mon_Hab, ")
            loComandoSeleccionar.AppendLine("           CASE WHEN Movimientos_Cuentas.Cod_Tip = 'Deb-TR' ")
            loComandoSeleccionar.AppendLine("           THEN 'Transferencia' ELSE Movimientos_Cuentas.Tipo END AS Tipo,")
            loComandoSeleccionar.AppendLine("           Movimientos_Cuentas.Comentario AS Nom_Pro")
            loComandoSeleccionar.AppendLine("FROM  Movimientos_Cuentas ")
            loComandoSeleccionar.AppendLine("   JOIN  Cuentas_Bancarias ON Movimientos_Cuentas.Cod_Cue = Cuentas_Bancarias.Cod_Cue")
            loComandoSeleccionar.AppendLine("WHERE (Movimientos_Cuentas.Cod_Tip = 'Deb-TR' OR Movimientos_Cuentas.Cod_Tip = 'Deb-ND')")
            ...
            loComandoSeleccionar.AppendLine("        Movimientos_Cuentas.Mon_Deb, ")
            loComandoSeleccionar.AppendLine("        Movimientos_Cuentas.Mon_Hab, ")
            loComandoSeleccionar.AppendLine("		 'IGTF:'				AS Tipo, ")
            loComandoSeleccionar.AppendLine("        'IMPUESTO A LAS TRANSACCIONES FINANCIERAS'		AS Nom_Pro")
            loComandoSeleccionar.AppendLine("FROM  Movimientos_Cuentas ")
            loComandoSeleccionar.AppendLine("WHERE (Movimientos_Cuentas.Cod_Tip = 'DEB-ID' AND Movimientos_Cuentas.Cod_Imp = '')")
            loComandoSeleccionar.AppendLine("   AND ((Movimientos_Cuentas.Fec_Ini BETWEEN @ldFechaInicio AND @ldFechaFin")
            loComandoSeleccionar.AppendLine("	AND (Movimientos_Cuentas.Fec_Cil NOT BETWEEN @ldFechaInicio AND @ldFechaFin")
            loComandoSeleccionar.AppendLine("       OR Movimientos_Cuentas.Doc_Cil     =   '0'))")
            loComandoSeleccionar.AppendLine("	    OR (Movimientos_Cuentas.Fec_Ini < @ldFechaInicio ")
            ...
            loComandoSeleccionar.AppendLine("UNION ALL")
            loComandoSeleccionar.AppendLine("")
            'DÉBITO POR IGTF QUE VIENE DE MOVIMIENTOS DE CUENTAS
            loComandoSeleccionar.AppendLine("SELECT  Movimientos_Cuentas.Documento, ")
            loComandoSeleccionar.AppendLine("        Movimientos_Cuentas.Cod_Cue, ")
            loComandoSeleccionar.AppendLine("        Movimientos_Cuentas.Fec_Ini, ")
            loComandoSeleccionar.AppendLine("        Movimientos_Cuentas.Referencia,")
            ...
            loComandoSeleccionar.AppendLine("        'IMPUESTO A LAS TRANSACCIONES FINANCIERAS'		AS Nom_Pro")
            loComandoSeleccionar.AppendLine("FROM  Movimientos_Cuentas ")
            loComandoSeleccionar.AppendLine("WHERE (Movimientos_Cuentas.Cod_Tip = 'DEB-ID' AND Movimientos_Cuentas.Tip_Ori = 'Movimientos_Cuentas')")
            loComandoSeleccionar.AppendLine("   AND ((Movimientos_Cuentas.Fec_Ini BETWEEN @ldFechaInicio AND @ldFechaFin")
            loComandoSeleccionar.AppendLine("	AND (Movimientos_Cuentas.Fec_Cil NOT BETWEEN @ldFechaInicio AND @ldFechaFin")
            loComandoSeleccionar.AppendLine("       OR Movimientos_Cuentas.Doc_Cil     =   '0'))")
            loComandoSeleccionar.AppendLine("	    OR (Movimientos_Cuentas.Fec_Ini < @ldFechaInicio ")
            ...
            loComandoSeleccionar.AppendLine("        Movimientos_Cuentas.Comentario		AS Nom_Pro")
            loComandoSeleccionar.AppendLine("FROM  Movimientos_Cuentas ")
            loComandoSeleccionar.AppendLine("WHERE   Movimientos_Cuentas.Cod_Con = 'E0234'")
            loComandoSeleccionar.AppendLine("       AND ((Movimientos_Cuentas.Fec_Ini BETWEEN @ldFechaInicio AND @ldFechaFin")
            loComandoSeleccionar.AppendLine("	   AND (Movimientos_Cuentas.Fec_Cil NOT BETWEEN @ldFechaInicio AND @ldFechaFin")
            loComandoSeleccionar.AppendLine("		OR Movimientos_Cuentas.Doc_Cil     =   '0'))")
            loComandoSeleccionar.AppendLine("	   OR (Movimientos_Cuentas.Fec_Ini < @ldFechaInicio ")
            ...
            loComandoSeleccionar.AppendLine("        Movimientos_Cuentas.Comentario		AS Nom_Pro")
            loComandoSeleccionar.AppendLine("FROM  Movimientos_Cuentas ")
            loComandoSeleccionar.AppendLine("WHERE   Movimientos_Cuentas.Cod_Con = 'C0002'")
            loComandoSeleccionar.AppendLine("       AND ((Movimientos_Cuentas.Fec_Ini BETWEEN @ldFechaInicio AND @ldFechaFin")
            loComandoSeleccionar.AppendLine("	   AND (Movimientos_Cuentas.Fec_Cil NOT BETWEEN @ldFechaInicio AND @ldFechaFin")
            loComandoSeleccionar.AppendLine("		OR Movimientos_Cuentas.Doc_Cil     =   '0'))")
            loComandoSeleccionar.AppendLine("	   OR (Movimientos_Cuentas.Fec_Ini < @ldFechaInicio ")
            ...
            loComandoSeleccionar.AppendLine("	   AND Movimientos_Cuentas.Status = 'Confirmado'")
            loComandoSeleccionar.AppendLine("")
            loComandoSeleccionar.AppendLine("SELECT		COALESCE(#tmpCuenta.Cod_Cue,'')						    AS Cod_Cue,")
            loComandoSeleccionar.AppendLine("           COALESCE(#tmpCuenta.Num_Cue,'')						    AS Num_Cue,")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpCuenta.Nom_Ban,'')						    AS Nom_Ban,")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpSaldoInicial.Mon_Sal_Ini, @lnCero) 		AS Saldo_Inicial,")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpConciliado.Sal_Ini_Con, @lnCero)			AS Sal_Ini_Con,")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpConciliado.Cargos,@lnCero) 				AS Cargos,")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpConciliado.Abonos,@lnCero) 				AS Abonos,")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpConciliado.Sal_Fin_Con,@lnCero)			AS Sal_Fin_Con,")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpNoConciliado.CargosNC,@lnCero) 			AS CargosNC,")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpNoConciliado.AbonosNC,@lnCero) 			AS AbonosNC,")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpNoConciliado.CargosNC,@lnCero) - ")
            loComandoSeleccionar.AppendLine("				COALESCE(#tmpNoConciliado.AbonosNC,@lnCero)			AS Sal_Fin_No_Con,")
            loComandoSeleccionar.AppendLine("			@ldSaldoEstado											AS Sal_Edo_Cta,	")
            loComandoSeleccionar.AppendLine("			(COALESCE(#tmpConciliado.Sal_Fin_Con, @lnCero) - ")
            loComandoSeleccionar.AppendLine("				@ldSaldoEstado)										AS Diferencia,	")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpConciliado.Sal_Fin_Con,@lnCero) + ")
            loComandoSeleccionar.AppendLine("               COALESCE(#tmpNoConciliado.Sal_Fin_No_Con,@lnCero)   AS Saldo_Libro,")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpConciliado.Abonos,@lnCero) + ")
            loComandoSeleccionar.AppendLine("               COALESCE(#tmpNoConciliado.AbonosNC,@lnCero)         AS Saldo_Abonos,")
            loComandoSeleccionar.AppendLine("			COALESCE(#tmpConciliado.Cargos,@lnCero) + ")
            loComandoSeleccionar.AppendLine("               COALESCE(#tmpNoConciliado.CargosNC,@lnCero)         AS Saldo_Cargos,")
            ...
            loComandoSeleccionar.AppendLine("ORDER BY #tmpConciliado.Cod_Cue, #tmpTransito.Fec_Ini")
            loComandoSeleccionar.AppendLine("")
