...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "rLibro_Compras_GrupoAlegria"
'-------------------------------------------------------------------------------------------'
Partial Class rLibro_Compras_GrupoAlegria
	...
	Try	
			...
			loComandoSeleccionar.AppendLine("			 	WHEN 'RETIVA'	THEN 'Retensión de I.V.A.'										" )
			...
			loComandoSeleccionar.AppendLine("						Cuentas_Pagar.Cod_Tip IN ('FACT', 'N/DB', 'RETIVA')")
			...
			loComandoSeleccionar.AppendLine(" 			AND Cuentas_Pagar.cod_tip IN ('FACT', 'N/CR', 'N/DB', 'RETIVA')")
		...
		'************************************************************************************************************************
		' Busca el documento de origen de las N/CR que provienen de devoluciones.												*   
		'************************************************************************************************************************
            loComandoSeleccionar.AppendLine("UPDATE		#tempCxP	")
            loComandoSeleccionar.AppendLine("SET		Documento_Afectado = CASE WHEN (Compras.Factura<>'') THEN Compras.Factura ELSE Compras.Documento END")
            loComandoSeleccionar.AppendLine("FROM		Cuentas_Pagar")
            loComandoSeleccionar.AppendLine("	JOIN	Renglones_dProveedores AS RDP")
            loComandoSeleccionar.AppendLine("		ON	RDP.Documento = Cuentas_Pagar.Doc_Ori")
            loComandoSeleccionar.AppendLine("		AND Cuentas_Pagar.Tip_Ori = 'Devoluciones_Proveedores'")
            loComandoSeleccionar.AppendLine("	JOIN	Compras ON Compras.Documento = RDP.Doc_Ori")
            loComandoSeleccionar.AppendLine("WHERE		Cuentas_Pagar.Cod_Tip = 'N/CR'")
            loComandoSeleccionar.AppendLine("	AND		#tempCxP.Cod_Tip = 'Nota de Credito'")
            loComandoSeleccionar.AppendLine("	AND		Cuentas_Pagar.Documento = #tempCxP.Documento ")
			loComandoSeleccionar.AppendLine("")
			loComandoSeleccionar.AppendLine("UPDATE		#tempCxP	")
			loComandoSeleccionar.AppendLine("SET		Documento_Afectado = Referencia")
			loComandoSeleccionar.AppendLine("WHERE		#tempCxP.Codigo_Tipo = 'N/CR'")
			loComandoSeleccionar.AppendLine("	AND		#tempCxP.Documento_Afectado = ''")
			loComandoSeleccionar.AppendLine("")
            loComandoSeleccionar.AppendLine("UPDATE		#tempCxP	")
            loComandoSeleccionar.AppendLine("SET			Documento = R.Num_Com,")
            loComandoSeleccionar.AppendLine("			Documento_Afectado = CASE WHEN (FC.Factura<>'') THEN FC.Factura ELSE FC.Documento END")
            loComandoSeleccionar.AppendLine("FROM		#tempCxP")
            loComandoSeleccionar.AppendLine("	JOIN	Retenciones_Documentos AS R")
            loComandoSeleccionar.AppendLine("		ON	R.Doc_Des = #tempCxP.Documento						")
            loComandoSeleccionar.AppendLine("		AND	R.Cla_Des = 'RETIVA'								")
            loComandoSeleccionar.AppendLine("		AND R.Tip_Des = 'Cuentas_Pagar'						")
            loComandoSeleccionar.AppendLine("	JOIN	Cuentas_Pagar AS FC")
            loComandoSeleccionar.AppendLine("		ON	R.Doc_Ori = FC.Documento						")
            loComandoSeleccionar.AppendLine("		AND	R.Cla_Ori = 'FACT'								")
            loComandoSeleccionar.AppendLine("		AND	FC.Cod_Tip = 'FACT'								")
            loComandoSeleccionar.AppendLine("		AND R.Tip_Ori = 'Cuentas_Pagar'						")
            loComandoSeleccionar.AppendLine("WHERE		#tempCxP.Codigo_Tipo = 'RETIVA'")
            loComandoSeleccionar.AppendLine("	AND		R.Num_Com <> ''")
            loComandoSeleccionar.AppendLine("")
            loComandoSeleccionar.AppendLine("")
            loComandoSeleccionar.AppendLine("")
			...
			If lcParametro6Desde.ToUpper() = "SI" then 
				...
				loComandoSeleccionar.AppendLine("ORDER BY " & lcOrdenamiento.Replace("Cuentas_Pagar.", " "))
            ...
            loObjetoReporte = cusAplicacion.goReportes.mCargarReporte("rLibro_Compras_GrupoAlegria", laDatosReporte2)
...
'-------------------------------------------------------------------------------------------'
' RJG: 25/10/11: Ajuste en presentación de Montos Otros (se consideran Exentos). Aplicación '
'				 de criterio para obtener número de documento fiscal y número de factura	'
'				 asociada en el caso de las N/CR. Filtradas las N/CR para mostrar solo las	'
'				 que sean de Devolución de Compras.											'
'-------------------------------------------------------------------------------------------'
' RJG: 02/11/11: Agregado filtro para no incluir las Ordenes de Pago generadas desde Punto	'
'				 de Venta. 																	'
'-------------------------------------------------------------------------------------------'
