...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "rLibro_Compras_SAKAN"
'-------------------------------------------------------------------------------------------'
Partial Class rLibro_Compras_SAKAN
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loConsulta.AppendLine("								Impuesto_Excluir	DECIMAL(28,10) DEFAULT(0)); ")
            loConsulta.AppendLine("")
            loConsulta.AppendLine("")
            ...
            loConsulta.AppendLine("FROM    (   SELECT ROW_NUMBER() OVER (PARTITION BY Periodo_Anterior ORDER BY (CASE WHEN Periodo_Anterior = 0 THEN Fec_Ini ELSE Fec_Doc END) ASC) Posicion,")
            ...
            loConsulta.AppendLine("")
            loConsulta.AppendLine("-- Exclusión de impuestos segun gaceta 6152")
            loConsulta.AppendLine("UPDATE	#tmpLibroCompras ")
            loConsulta.AppendLine("SET     Impuesto_Excluir = Excluir.Mon_Imp1")
            loConsulta.AppendLine("FROM (  SELECT X.Documento, X.Cod_Tip, SUM(X.Mon_Imp1) Mon_Imp1 ")
            loConsulta.AppendLine("        FROM (")
            loConsulta.AppendLine("            SELECT      Cuentas_Pagar.Documento, Cuentas_Pagar.Cod_Tip, ")
            loConsulta.AppendLine("                        (CASE WHEN Cuentas_Pagar.Cod_Tip = 'N/CR' ")
            loConsulta.AppendLine("                         THEN -Cuentas_Pagar.Mon_Imp1 ELSE Cuentas_Pagar.Mon_Imp1 END) Mon_Imp1")
            loConsulta.AppendLine("            FROM        #tmpLibroCompras")
            loConsulta.AppendLine("                JOIN    Cuentas_Pagar ")
            loConsulta.AppendLine("                    ON  Cuentas_Pagar.Documento = #tmpLibroCompras.Documento")
            loConsulta.AppendLine("                    AND Cuentas_Pagar.Cod_Tip = #tmpLibroCompras.Codigo_Tipo")
            loConsulta.AppendLine("                    AND Cuentas_Pagar.Automatico = 0")
            loConsulta.AppendLine("                    AND CAST(Not_Sta AS XML).value('(clasificacion/logico1)[1]', 'VARCHAR(MAX)') = 'true'")
            loConsulta.AppendLine("            UNION ALL  ")
            loConsulta.AppendLine("            SELECT      Compras.Documento, 'FACT' AS Cod_Tip, Compras.Mon_Imp1 Mon_Imp1")
            loConsulta.AppendLine("            FROM        #tmpLibroCompras")
            loConsulta.AppendLine("                JOIN    Compras ")
            loConsulta.AppendLine("                    ON  Compras.Documento = #tmpLibroCompras.Documento")
            loConsulta.AppendLine("            WHERE       CAST(Not_Sta AS XML).value('(clasificacion/logico1)[1]', 'VARCHAR(MAX)') = 'true'")
            loConsulta.AppendLine("            UNION ALL  ")
            loConsulta.AppendLine("            SELECT      Renglones_Compras.Documento, 'FACT' AS Cod_Tip, Renglones_Compras.Mon_Imp1 Mon_Imp1")
            loConsulta.AppendLine("            FROM        #tmpLibroCompras")
            loConsulta.AppendLine("                JOIN    Compras ")
            loConsulta.AppendLine("                    ON  Compras.Documento = #tmpLibroCompras.Documento")
            loConsulta.AppendLine("                    AND CAST(Compras.Not_Sta AS XML).value('(clasificacion/logico1)[1]', 'VARCHAR(MAX)') <> 'true'")
            loConsulta.AppendLine("                JOIN    Renglones_Compras ")
            loConsulta.AppendLine("                    ON  Renglones_Compras.Documento = Compras.Documento")
            loConsulta.AppendLine("                    AND CAST(Renglones_Compras.Not_Sta AS XML).value('(clasificacion/logico1)[1]', 'VARCHAR(MAX)') = 'true'")
            loConsulta.AppendLine("        ) X ")
            loConsulta.AppendLine("        GROUP BY X.Documento, X.Cod_Tip")
            loConsulta.AppendLine("    ) Excluir")
            loConsulta.AppendLine("WHERE   #tmpLibroCompras.Documento = Excluir.Documento")
            loConsulta.AppendLine("    AND #tmpLibroCompras.Codigo_Tipo = Excluir.Cod_Tip;")
            loConsulta.AppendLine("")
            ...
            loConsulta.AppendLine("        Control                                         AS Control,")
            loConsulta.AppendLine("        Factura                                         AS Factura_Pro,")
            ...
            loConsulta.AppendLine("        Mon_Imp - Impuesto_Excluir                      AS Mon_Imp,")
            ...
            loConsulta.AppendLine("        Impuesto_Excluir                                AS Impuesto_Excluir")
            loConsulta.AppendLine("FROM    #tmpLibroCompras	")
            loConsulta.AppendLine("ORDER BY Periodo_Anterior ASC, ")
            loConsulta.AppendLine("        (CASE WHEN Periodo_Anterior = 0 THEN Fec_Ini ELSE Fec_Doc END) ASC")
            loConsulta.AppendLine("")
            loConsulta.AppendLine("")
            loConsulta.AppendLine("")
            ...
            loObjetoReporte = cusAplicacion.goReportes.mCargarReporte("rLibro_Compras_SAKAN", laDatosReporte )
            ...
            If (Me.Request.QueryString("salida").ToLower = "xls") Then
                ...
                Dim lcFileName As String = Me.Server.MapPath("~\Administrativo\Temporales\rLibro_Compras_SAKAN_" & Guid.NewGuid().ToString("N") & ".xls")
                ...
                Me.Response.AppendHeader("content-disposition", "attachment; filename=rLibro_Compras_SAKAN.xls")
            ...
            Me.crvrrLibro_Compras_SAKAN.ReportSource = loObjetoReporte
