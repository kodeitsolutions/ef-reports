...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "rTrabajadores_FAOV"
'-------------------------------------------------------------------------------------------'
Partial Class rTrabajadores_FAOV
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'laConceptosDevengadoFAOV.Add("'Q023'")
        'laConceptosDevengadoFAOV.Add("'Q123'")
        'laConceptosDevengadoFAOV.Add("'Q423'")

        lcConceptosDevengadoFAOV = String.Join(",", laConceptosDevengadoFAOV.ToArray(GetType(String)))
        ...
        lcConceptosFAOV_Trabajador = String.Join(",", laConceptosFAOV_Trabajador.ToArray(GetType(String)))
        ...
        lcConceptosFAOV_Patrono = String.Join(",", laConceptosFAOV_Patrono.ToArray(GetType(String)))

        Try

            loConsulta.AppendLine("")
            loConsulta.AppendLine("SELECT      ROW_NUMBER() ")
            loConsulta.AppendLine("                OVER(ORDER BY Trabajadores.Cod_Tra ASC)     AS Numero,")
            loConsulta.AppendLine("            CAST( " & lcParametro0Desde & " AS DATE)        AS Periodo,")
            ...
            loConsulta.AppendLine("                            END)                                AS Observaciones,")
            loConsulta.AppendLine("                        SUM(CASE WHEN DAY(Recibos.Fec_Fin)<16")
            loConsulta.AppendLine("                                AND Renglones_Recibos.Cod_Con IN (" & lcConceptosDevengadoFAOV & ")")
            ...
            loConsulta.AppendLine("                    AND     MONTH(Recibos.fec_fin) = MONTH(CAST( " & lcParametro0Desde & " AS DATE))")
            loConsulta.AppendLine("                    AND     YEAR(Recibos.fec_fin) = YEAR(CAST( " & lcParametro0Desde & " AS DATE))")
            ...
            loConsulta.AppendLine("WHERE   Trabajadores.Cod_Tra BETWEEN " & lcParametro1Desde)
            ...
            loConsulta.AppendLine("        AND Trabajadores.Cod_Suc BETWEEN " & lcParametro6Desde)
            ...
            loObjetoReporte = cusAplicacion.goReportes.mCargarReporte("rTrabajadores_FAOV", laDatosReporte)

            Me.mTraducirReporte(loObjetoReporte)

            Me.mFormatearCamposReporte(loObjetoReporte)

            Me.crvrTrabajadores_FAOV.ReportSource = loObjetoReporte
...
' Fin del codigo.                                                                           '
'-------------------------------------------------------------------------------------------'
' RJG: 25/08/14: Codigo inicial.                                                            '
'-------------------------------------------------------------------------------------------'
' RJG: 17/09/14: Programación del envío a TXT.                                              '
'-------------------------------------------------------------------------------------------'
' RJG: 09/06/14: Se agregaron los campos R403 y U403 a las listas de conceptos de retención '
'                al empleado y aporte patronal.                                             '
'-------------------------------------------------------------------------------------------'
