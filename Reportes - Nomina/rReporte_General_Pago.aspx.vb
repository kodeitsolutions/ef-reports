...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "rReporte_General_Pago"
'-------------------------------------------------------------------------------------------'
Partial Class rReporte_General_Pago
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loConsulta.AppendLine("            MAX(Renglones_Recibos.val_car),")
            loConsulta.AppendLine("            (CASE WHEN PATINDEX('%[^0-9/,\. %-]%',MAX(Renglones_Recibos.val_car))>0")
            loConsulta.AppendLine("            THEN SUBSTRING(MAX(Renglones_Recibos.val_car),")
            loConsulta.AppendLine("                 PATINDEX('%[^0-9/,\. %-]%',MAX(Renglones_Recibos.val_car)),100)   ")
            loConsulta.AppendLine("            ELSE '' END)                                                        AS Unidad,")
            loConsulta.AppendLine("            SUM(Renglones_Recibos.mon_net)                                      AS Asignacion,")
            ...
            loObjetoReporte = cusAplicacion.goReportes.mCargarReporte("rReporte_General_Pago", laDatosReporte)

            Me.mTraducirReporte(loObjetoReporte)

            Me.mFormatearCamposReporte(loObjetoReporte)

            Me.crvrReporte_General_Pago.ReportSource = loObjetoReporte
...
' Fin del codigo																			'
'-------------------------------------------------------------------------------------------'
' RJG: 12/08/14: Codigo inicial, a partir de rReporte_General_Pago_SID.						'
'-------------------------------------------------------------------------------------------'
