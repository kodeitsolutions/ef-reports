...
'-------------------------------------------------------------------------------------------'
' Inicio de clase "rLiquidacionesRecibos_Fechas"
'-------------------------------------------------------------------------------------------'
Partial Class rLiquidacionesRecibos_Fechas
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loConsulta.AppendLine("        COALESCE(Liquidacion.Liquidacion_Documento,'')   AS Liquidacion_Documento,	")
            loConsulta.AppendLine("        COALESCE(Liquidaciones.Motivo, '')	        	AS Cod_Mot,		")
            ...
            loConsulta.AppendLine("    LEFT JOIN(  SELECT  Renglones_Recibos.Documento AS Liquidacion_recibo,")
            loConsulta.AppendLine("                        Renglones_Recibos.Doc_Ori AS Liquidacion_Documento")
            loConsulta.AppendLine("                FROM    Renglones_Recibos")
            loConsulta.AppendLine("	               WHERE   Renglones_Recibos.Tip_Ori = 'Liquidaciones'")
            loConsulta.AppendLine("	               GROUP BY Renglones_Recibos.Documento, Renglones_Recibos.Doc_Ori")
            loConsulta.AppendLine("	        ) Liquidacion")
            loConsulta.AppendLine("        ON  Liquidacion.Liquidacion_recibo = Recibos.Documento")
            loConsulta.AppendLine("    LEFT JOIN Liquidaciones ")
            loConsulta.AppendLine("        ON Liquidaciones.Documento = Liquidacion.Liquidacion_Documento")
            loConsulta.AppendLine("    LEFT JOIN Motivos ")
            loConsulta.AppendLine("		ON	Motivos.Cod_Mot = Liquidaciones.Motivo")
            loConsulta.AppendLine("WHERE    Recibos.Cod_Con = '92'")
            ...
            loObjetoReporte = cusAplicacion.goReportes.mCargarReporte("rLiquidacionesRecibos_Fechas", laDatosReporte)

            Me.mTraducirReporte(loObjetoReporte)

            Me.mFormatearCamposReporte(loObjetoReporte)

            Me.crvrLiquidacionesRecibos_Fechas.ReportSource = loObjetoReporte
