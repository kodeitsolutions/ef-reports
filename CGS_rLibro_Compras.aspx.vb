...
Partial Class CGS_rLibro_Compras
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            loComandoSeleccionar.AppendLine("				CAST(ISNULL((SELECT CAST(Dis_Imp AS XML)")
            loComandoSeleccionar.AppendLine("							FROM Cuentas_Pagar AS C_P ")
            loComandoSeleccionar.AppendLine("							WHERE C_P.Documento = Cuentas_Pagar.Documento AND C_P.Cod_Tip = Cuentas_Pagar.Cod_Tip)")
            loComandoSeleccionar.AppendLine("					.value('(/impuestos/impuesto/base)[1]', 'VARCHAR(MAX)'),0)AS DECIMAL (28,10))                  AS Base_Imponible,")
            loComandoSeleccionar.AppendLine("				CAST(ISNULL((SELECT CAST(Dis_Imp AS XML)")
            loComandoSeleccionar.AppendLine("							FROM Cuentas_Pagar AS C_P ")
            loComandoSeleccionar.AppendLine("							WHERE C_P.Documento = Cuentas_Pagar.Documento AND C_P.Cod_Tip = Cuentas_Pagar.Cod_Tip)")
            loComandoSeleccionar.AppendLine("					.value('(/impuestos/impuesto/porcentaje)[1]', 'VARCHAR(MAX)'),0)AS DECIMAL (28,10))			        AS Impuesto,")
            loComandoSeleccionar.AppendLine("				CAST(ISNULL((SELECT CAST(Dis_Imp AS XML)")
            loComandoSeleccionar.AppendLine("							FROM Cuentas_Pagar AS C_P ")
            loComandoSeleccionar.AppendLine("							WHERE C_P.Documento = Cuentas_Pagar.Documento AND C_P.Cod_Tip = Cuentas_Pagar.Cod_Tip)")
            loComandoSeleccionar.AppendLine("					.value('(/impuestos/impuesto/monto)[1]', 'VARCHAR(MAX)'),0)AS DECIMAL (28,10))                  AS Monto_Imp,")
            loComandoSeleccionar.AppendLine("				CAST(ISNULL((SELECT CAST(Dis_Imp AS XML)")
            loComandoSeleccionar.AppendLine("							FROM Cuentas_Pagar AS C_P ")
            loComandoSeleccionar.AppendLine("							WHERE C_P.Documento = Cuentas_Pagar.Documento AND C_P.Cod_Tip = Cuentas_Pagar.Cod_Tip)")
            loComandoSeleccionar.AppendLine("					.value('(/impuestos/impuesto/base)[2]', 'VARCHAR(MAX)'),0)AS DECIMAL (28,10))                  AS Base_Imponible2,")
            loComandoSeleccionar.AppendLine("				CAST(ISNULL((SELECT CAST(Dis_Imp AS XML)")
            loComandoSeleccionar.AppendLine("							FROM Cuentas_Pagar AS C_P ")
            loComandoSeleccionar.AppendLine("							WHERE C_P.Documento = Cuentas_Pagar.Documento AND C_P.Cod_Tip = Cuentas_Pagar.Cod_Tip)")
            loComandoSeleccionar.AppendLine("					.value('(/impuestos/impuesto/porcentaje)[2]', 'VARCHAR(MAX)'),0)AS DECIMAL (28,10))			        AS Impuesto2,")
            loComandoSeleccionar.AppendLine("				CAST(ISNULL((SELECT CAST(Dis_Imp AS XML)")
            loComandoSeleccionar.AppendLine("							FROM Cuentas_Pagar AS C_P ")
            loComandoSeleccionar.AppendLine("							WHERE C_P.Documento = Cuentas_Pagar.Documento AND C_P.Cod_Tip = Cuentas_Pagar.Cod_Tip)")
            loComandoSeleccionar.AppendLine("					.value('(/impuestos/impuesto/monto)[2]', 'VARCHAR(MAX)'),0)AS DECIMAL (28,10))                  AS Monto_Imp2,")
